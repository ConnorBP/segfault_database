...
if [ $choice -eq 1 ] ; then
	...
	if [ $choice -eq 2 ] ; then
		...
		sudo pm2 start "$root/sfdb_run.sh" --interpreter=bash --name=SegfaultDatabase
		sudo pm2 startup
		sudo pm2 save
		echo ""
		echo "If you did everything correctly, pm2 should have started up Nadeko! Please use sudo pm2 info Nadeko to check. You can view pm2 logs with sudo pm2 logs Nadeko"	
			...
			if [ $choice -eq 4 ] ; then
				...
				clear
				echo "1. Run in pm2 with Auto Restart normally without updating sfdb."
				echo "2. Run in pm2 with Auto Restart and update sfdb."
				echo "3. Run sfdb in pm2 normally without Auto Restart."
				echo "4. Exit"
				echo -n "Choose [1] to Run sfdb in pm2 with auto restart on "die" command without updating itself, [2] to Run in pm2 with Auto Updating on restart after using "die" command, and [3] to run without any auto restarts or auto-updates."
				choice=5
			fi
		fi
	fi
